...
void game_run(void) {
    ...
    bool encounter_music_playing = false;
    
    // Load and play initial room music
    ...
    audio_play_music();
    audio_set_music_volume(64); // 50% volume
    ...
    while (running) {
        ...
        if (near_professor && !encounter_music_playing) {
            // Switch to encounter music
            audio_stop_music();
            audio_load_music("assets/music/encounter.ogg");
            audio_play_music();
            audio_set_music_volume(64);
            encounter_music_playing = true;
            printf("🎵 Encounter music started!\n");
        } else if (!near_professor && encounter_music_playing) {
            // Switch back to room music
            audio_stop_music();
            audio_load_music(current_music_path);
            audio_play_music();
            audio_set_music_volume(64);
            encounter_music_playing = false;
        ...
        if (!player.is_moving) {  // Only check when player stops moving
